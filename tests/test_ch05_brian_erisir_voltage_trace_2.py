...
def test_erisir_voltage_trace_2_matches_matlab():
    ...
    current = get_step_current(0, 100, ns.b2.ms, 7.0 * ns.b2.uA)
    sm = ns.simulate_Erisir_neuron(current, 100 * ns.b2.ms, n_power=4)
    t = sm.t / ns.b2.ms
    v = sm.vm[0] / ns.b2.mV

    ref = run_matlab_script(
        ROOT / "matlab" / "05" / "ERISIR_VOLTAGE_TRACE_2", "make_figure.m", ["t", "v"]
    )
