...
def test_chapter05_notebook_wb_voltage_trace_1996():
    ...
    t, v = ns.simulate_wb_voltage_trace_1996()
    assert t.shape == v.shape
    assert np.all(np.isfinite(v))
    assert v.max() > 0  # the WB 1996 neuron should spike at the default i_ext=0.75
