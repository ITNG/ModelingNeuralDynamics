...
@pytest.mark.slow
def test_s_slow_buildup_matches_matlab():
    ns = load_notebook_definitions_as_module(ROOT / "brian" / "chapter20.ipynb")
    sm = ns.simulate_RTM_neuron_q_s(0.2 * ns.b2.uA, tau_r=100 * ns.b2.ms, tau_d=500 * ns.b2.ms,
                                     tau_d_q=1 * ns.b2.ms, simulation_time=2000 * ns.b2.ms)

    ref = run_matlab_script(ROOT / "matlab" / "20" / "S_SLOW_BUILDUP", "make_figure.m", ["t", "v", "s"])
