...
num_e=200; num_i=50;
sigma_e=0.00; i_ext_e=1.4*ones(num_e,1).*(1+sigma_e*randn(num_e,1)); 
...
axis([t_final-200,t_final,0,num_e+num_i+1]); 

toc;

tic;
% Define new network parameters: 


p_ee=1/num_e; p_ei=1/num_e; p_ie=1/num_i; p_ii=1/num_i;
...
for j=1:num_i,
    i_vec(1)=ceil(rand(1,1)*num_e);
    for m=2:ni,
        i0=i_vec(1);
        while max(abs(i_vec-i0)==0),
            i0=ceil(rand(1,1)*num_e);
        end;
        i_vec(m)=i0;
    end;
...
for i=1:num_e,
    j_vec(1)=ceil(rand(1,1)*num_i);
    for m=2:ni,
        j0=j_vec(1);
        while max(abs(j_vec-j0)==0),
            j0=ceil(rand(1,1)*num_i);
        end;
        j_vec(m)=j0;
    end;
