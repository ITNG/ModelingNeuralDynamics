...
N=200;                          % number of phi-values considered
dt=0.01; dt05=dt/2; 
...
for ijk=1:3,
    ...
    set(gca,'Fontsize',12);
    axis([0,1,0,max(g_vec)*2]);
    str_g_syn=num2str(g_syn);
    title(['$\overline{g}_{\rm syn}=$',str_g_syn],'Fontsize',16)
    set(gca,'Ytick',[0.04,0.08]/2^(ijk-1));
    if ijk==1,
