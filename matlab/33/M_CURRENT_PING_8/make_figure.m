...
g_m=0.5; % M-current conductance; the only parameter of the neuronal model,
         % other than external drive, not explicitly written into the code.
g_hat_ee=0.35; g_hat_ei=0.5; g_hat_ie=0.5; g_hat_ii=0.5; 
...
% Define recurrent excitation among E_P-cells:

g_ee=zeros(num_e,num_e);
for i=num_e/2+1:num_e,
    for j=num_e/2+1:num_e,
        g_ee(i,j)=g_hat_ee/(num_e/2);
    end;
end;
...
i_ext_e(num_e/2+1:num_e)=i_ext_e(num_e/2+1:num_e)*1.20;
