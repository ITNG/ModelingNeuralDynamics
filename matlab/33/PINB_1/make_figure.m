...
tau_r_i=0.5; tau_peak_i=0.5; tau_d_ii=9; tau_d_ie=90;
t_final=400;   % Time (in ms) simulated. 
...
z=zeros(num_i,1); q_ii=z; s_ii=z; q_ie=z; s_ie=z;
...
for k=1:m_steps,
	...
	v_e_inc=0.1*(-67-v_e)+80*n_e.^4.*(-100-v_e)+100*m_e.^3.*h_e.*(50-v_e) ...
               +(g_ee'*s_e).*(v_rev_e-v_e)+(g_ie'*s_ie).*(v_rev_i-v_e) ...
               +i_ext_e;
	...
	v_i_inc=0.1*(-65-v_i)+9*n_i.^4.*(-90-v_i)+35*m_i.^3.*h_i.*(55-v_i) ...
               +(g_ei'*s_e).*(v_rev_e-v_i)+(g_ii'*s_ii).*(v_rev_i-v_i) ...
               +i_ext_i;
	...
	v_e_inc=0.1*(-67-v_e_tmp)+80*n_e_tmp.^4.*(-100-v_e_tmp)+100*m_e_tmp.^3.*h_e_tmp.*(50-v_e_tmp) ...
               +(g_ee'*s_e_tmp).*(v_rev_e-v_e_tmp)+(g_ie'*s_ie_tmp).*(v_rev_i-v_e_tmp) ...
               +i_ext_e;
	...
	v_i_inc=0.1*(-65-v_i_tmp)+9*n_i_tmp.^4.*(-90-v_i_tmp)+35*m_i_tmp.^3.*h_i_tmp.*(55-v_i_tmp) ...
               +(g_ei'*s_e_tmp).*(v_rev_e-v_i_tmp)+(g_ii'*s_ii_tmp).*(v_rev_i-v_i_tmp) ...
               +i_ext_i;
