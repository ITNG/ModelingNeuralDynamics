...
for ijk=1:length(alpha_vec),
    ...
    C_vec(ijk)=mean(y);
    err_vec(ijk)=(max(y)-min(y))/mean(y);
end;
ind=find(err_vec==min(err_vec)); ind=min(ind);
alpha=alpha_vec(ind);
I_c=I_c_low*alpha+I_c_high*(1-alpha)
C=C_vec(ind)
err=err_vec(ind);
