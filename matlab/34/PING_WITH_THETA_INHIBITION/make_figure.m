...
P=125; g_ex=0.2; alpha_ex=@(t) exp(-10*sin(pi*t/P).^2);
...
lfp_v=zeros(m_steps+1,1); lfp_v(1)=mean(v_e);
lfp_s=zeros(m_steps+1,1); lfp_s(1)=mean(s_e);
...
for k=1:m_steps,
	...
	v_e_inc=0.1*(-67-v_e)+80*n_e.^4.*(-100-v_e)+100*m_e.^3.*h_e.*(50-v_e) ...
               ...
               +i_ext_e ...
               +g_ex*alpha_ex((k-1)*dt)*(v_rev_i-v_e);
	...
	v_e_inc=0.1*(-67-v_e_tmp)+80*n_e_tmp.^4.*(-100-v_e_tmp)+100*m_e_tmp.^3.*h_e_tmp.*(50-v_e_tmp) ...
               ...
               +i_ext_e ...
               +g_ex*alpha_ex((k-1/2)*dt)*(v_rev_i-v_e_tmp);
