...
phi_vec = np.arange(1, N) / N  # phi_vec = [1:N-1]/N, length N-1
...
# Calculate G from the computed g, by composing F=f(1-.) with itself
# (f(phi)=phi+g(phi)), via linear interpolation on the phi-grid.
dphi = 1 / N
