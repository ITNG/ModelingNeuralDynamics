...
def firing_rate(i_ext, g_store, stop_at_first_spike=False):
...
def bisect_threshold(g_store, target, stop_at_first_spike=False):
    ...
    while (i_ext_right - i_ext_left) / ((i_ext_right + i_ext_left) / 2) > epsilon:
        ...
        f = firing_rate(i_ext, g_store, stop_at_first_spike)
        if f < target:
            i_ext_left = i_ext
        else:
            i_ext_right = i_ext
...
for ijk, g_bar in enumerate(g_bar_vec):
    ...
    I_L = bisect_threshold(g_store, target=1e-9, stop_at_first_spike=True)
