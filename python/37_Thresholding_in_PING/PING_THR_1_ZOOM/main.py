...
num_e = 200
num_i = 50
i_ext_e = 1.2 + np.arange(1, num_e + 1) / num_e * 0.8  # deterministic ramp
sigma_i = 0.00
i_ext_i = 0.5 * np.ones(num_i) * (1 + sigma_i * rng.standard_normal(num_i))
g_hat_ee, g_hat_ei, g_hat_ie, g_hat_ii = 0., 0.5, 1.5, 0.25
...
def simulate(t_final=t_final):
    ...
    return (np.array(t_e_spikes), np.array(i_e_spikes),
            np.array(t_i_spikes), np.array(i_i_spikes), lfp)


t_e_spikes, i_e_spikes, t_i_spikes, i_i_spikes, lfp = simulate()

if __name__ == "__main__":
    # zoom into E-cells 72-78 (1-indexed), i.e. python indices 71-77
    fig, ax = plt.subplots(figsize=(8, 5))
    mask = (i_e_spikes >= 71) & (i_e_spikes <= 77)
    ax.plot(t_e_spikes[mask], i_e_spikes[mask] + 1, '.r', markersize=12)
    ax.axis([50, t_final, 71.5, 78.5])
    ax.set_xlabel('$t$ [ms]')
