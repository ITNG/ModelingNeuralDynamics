...
def tau_w(v):
    return 400.0 / (3.3 * exp((v + 35.0) / 20.0) + exp(-(v + 35.0) / 20.0))


def derivative(x0, t):
    '''
    define Traub Model with an M-current, at rest (i_ext=0)

    m is not its own state -- matlab/09/RTM_M_RESTING/make_figure.m
