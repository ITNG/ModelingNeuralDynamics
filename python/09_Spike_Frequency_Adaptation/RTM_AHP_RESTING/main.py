...
def derivative(x0, t):
    '''
    define Traub Model with a calcium-dependent AHP current, at rest
    (i_ext=0)

    m is not its own state -- matlab/09/RTM_AHP_RESTING/make_figure.m
    always sets m = m_inf(v) (quasi-static), never integrates it as an
    ODE. The [Ca2+] time constant is a plain constant (80), not the
    voltage-dependent tau_w.
    '''
    v, n, h, ca = x0
    m = m_inf(v)
    dv = i_ext - g_na * h * m ** 3 * \
