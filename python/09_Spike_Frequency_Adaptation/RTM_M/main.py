...
def derivative(x0, t):
    '''
    define Traub Model with an M-current

    m is not its own state -- matlab/09/RTM_M/make_figure.m always sets
    m = m_inf(v) (quasi-static), never integrates it as an ODE.
    '''
    v, n, h, w = x0
    m = m_inf(v)
    dv = i_ext - g_na * h * m ** 3 * \
