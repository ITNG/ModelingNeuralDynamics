...
Synapses can change strength through a pulse train even with fixed anatomy. These examples introduce pulse-driven resource and utilization dynamics, then show depressing and facilitating synapses in RTM and WB neurons.
...
With resources \(x\) and utilization \(u\), release is proportional to \(ux\); depression lowers \(x\) after events and facilitation raises \(u\). The resulting synaptic gate enters the conductance current \(gs(E_{\rm rev}-v)\).

## Code examples

- [`PULSES`](PULSES/) plots the pulse train driving the synaptic dynamics.
...
Track synaptic strength over pulses, not only voltage. Depression should weaken later events until recovery; facilitation can enhance them transiently. Compare RTM and WB to separate synaptic from intrinsic-neuron effects.
