...
sigma_i = 0.00
i_ext_i = 0.0 * np.ones(num_i) * (1 + sigma_i * rng.standard_normal(num_i))
...
# PINB (PING with I-cell "burst"): the I-cells drive I-I synapses (fast,
# tau_d_ii) and I-E synapses (slow, tau_d_ie) with independent gating
# variables, even though both are driven by the same I-cell spikes.
tau_r_i, tau_peak_i, tau_d_ii, tau_d_ie = 0.5, 0.5, 9., 90.
t_final = 400.
dt = 0.01
dt05 = dt / 2
...
for k in range(1, m_steps + 1):
    v_e_inc = (0.1 * (-67 - v_e) + 80 * n_e ** 4 * (-100 - v_e) + 100 * m_e ** 3 * h_e * (50 - v_e)
                + (g_ee.T @ s_e) * (v_rev_e - v_e) + (g_ie.T @ s_ie) * (v_rev_i - v_e) + i_ext_e)
    ...
    v_i_inc = (0.1 * (-65 - v_i) + 9 * n_i ** 4 * (-90 - v_i) + 35 * m_i ** 3 * h_i * (55 - v_i)
                + (g_ei.T @ s_e) * (v_rev_e - v_i) + (g_ii.T @ s_ii) * (v_rev_i - v_i) + i_ext_i)
    n_i_inc = (n_i_inf(v_i) - n_i) / tau_n_i(v_i)
    h_i_inc = (h_i_inf(v_i) - h_i) / tau_h_i(v_i)
    q_ii_inc = (1 + tanh(v_i / 10)) / 2 * (1 - q_ii) / 0.1 - q_ii / tau_dq_ii
    q_ie_inc = (1 + tanh(v_i / 10)) / 2 * (1 - q_ie) / 0.1 - q_ie / tau_dq_ie
    s_ii_inc = q_ii * (1 - s_ii) / tau_r_i - s_ii / tau_d_ii
    s_ie_inc = q_ie * (1 - s_ie) / tau_r_i - s_ie / tau_d_ie
    ...
    q_ii_tmp = q_ii + dt05 * q_ii_inc
    s_ii_tmp = s_ii + dt05 * s_ii_inc
    q_ie_tmp = q_ie + dt05 * q_ie_inc
    s_ie_tmp = s_ie + dt05 * s_ie_inc

    v_e_inc = (0.1 * (-67 - v_e_tmp) + 80 * n_e_tmp ** 4 * (-100 - v_e_tmp)
                + 100 * m_e_tmp ** 3 * h_e_tmp * (50 - v_e_tmp)
                + (g_ee.T @ s_e_tmp) * (v_rev_e - v_e_tmp) + (g_ie.T @ s_ie_tmp) * (v_rev_i - v_e_tmp) + i_ext_e)
    ...
    v_i_inc = (0.1 * (-65 - v_i_tmp) + 9 * n_i_tmp ** 4 * (-90 - v_i_tmp)
                ...
                + (g_ei.T @ s_e_tmp) * (v_rev_e - v_i_tmp) + (g_ii.T @ s_ii_tmp) * (v_rev_i - v_i_tmp) + i_ext_i)
    n_i_inc = (n_i_inf(v_i_tmp) - n_i_tmp) / tau_n_i(v_i_tmp)
    h_i_inc = (h_i_inf(v_i_tmp) - h_i_tmp) / tau_h_i(v_i_tmp)
    q_ii_inc = (1 + tanh(v_i_tmp / 10)) / 2 * (1 - q_ii_tmp) / 0.1 - q_ii_tmp / tau_dq_ii
    s_ii_inc = q_ii_tmp * (1 - s_ii_tmp) / tau_r_i - s_ii_tmp / tau_d_ii
    q_ie_inc = (1 + tanh(v_i_tmp / 10)) / 2 * (1 - q_ie_tmp) / 0.1 - q_ie_tmp / tau_dq_ie
    s_ie_inc = q_ie_tmp * (1 - s_ie_tmp) / tau_r_i - s_ie_tmp / tau_d_ie
    ...
    h_i = h_i + dt * h_i_inc
    n_i = n_i + dt * n_i_inc
    q_ii = q_ii + dt * q_ii_inc
    s_ii = s_ii + dt * s_ii_inc
    q_ie = q_ie + dt * q_ie_inc
    s_ie = s_ie + dt * s_ie_inc
