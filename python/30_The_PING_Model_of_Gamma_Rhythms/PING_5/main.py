...
def make_fixed_degree_connectivity(p_ei, p_ie, p_ii, ni=1):
    ...
    (feeding g_ii); g_ee stays all-zero (matlab's script never populates
    g_ee in this block).

    matlab's script normalizes these weights using p_ei/p_ie/p_ii, but
    by this point in the script those variables have already been
    overwritten by the *previous* (sparse-random) block's values
    (1/num_e, 1/num_i, 1/num_i) -- not panel 1's p=0.5. The caller must
    pass those same overwritten values here to match.'''
...
def simulate(g_ee, g_ei, g_ie, g_ii, t_final=t_final):
    ...
    t_e_spikes, i_e_spikes, t_i_spikes, i_i_spikes = _ping_step_loop(
        m_steps_, dt, dt05, num_e, num_i,
        v_rev_e, v_rev_i, tau_r_e, tau_d_e, tau_dq_e,
        tau_r_i, tau_d_i, tau_dq_i,
        i_ext_e, i_ext_i, g_ee, g_ei, g_ie, g_ii,
        v_e, h_e, n_e, m_e, q_e, s_e,
