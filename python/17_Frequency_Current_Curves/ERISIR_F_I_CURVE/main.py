...
def run_to_frequency(i_ext, v, m, h, n):
    '''Heun/RK2 integration (plain floats) of the Erisir model at fixed
    i_ext, starting from the state left over by the previous i_ext (this
    continuation is what makes the forward/backward sweep show hysteresis).
    Returns (f, v, m, h, n) -- f=0 if v settles to rest within 10s, else
    the frequency from the 3rd/4th spike interval.'''
    win_maxv = win_minv = v
    win_maxm = win_minm = m
    win_maxh = win_minh = h
    win_maxn = win_minn = n
    ...
    for k in range(1, t_max_steps + 1):
        v_prev = v
        v_inc = (g_na * m ** 3 * h * (v_na - v) + g_k * n ** 2 * (v_k - v) + g_l * (v_l - v) + i_ext) / c
        ...
        v_inc = (g_na * m_tmp ** 3 * h_tmp * (v_na - v_tmp) + g_k * n_tmp ** 2 * (v_k - v_tmp)
