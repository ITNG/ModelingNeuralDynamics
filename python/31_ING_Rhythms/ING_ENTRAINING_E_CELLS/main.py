...
num_e, num_i = 400, 100
sigma_e = 0.10
i_ext_e = 1.5 * (1 + sigma_e * rng.standard_normal(num_e))
sigma_i = 0.05
i_ext_i = 1.5 * (1 + sigma_i * rng.standard_normal(num_i))
g_hat_ee, g_hat_ei, g_hat_ie, g_hat_ii = 0., 0., 0.50, 0.50
g_hat_gap, p_gap = 0.1, 0.05
p_ee, p_ei, p_ie, p_ii = 1.0, 0.5, 0.5, 0.5
...
def simulate(i_ext_e_local, t_final=t_final):
    ...
    for k in range(1, m_steps_ + 1):
        v_e_inc = (0.1 * (-67 - v_e) + 80 * n_e ** 4 * (-100 - v_e) + 100 * m_e ** 3 * h_e * (50 - v_e)
                    + (g_ee.T @ s_e) * (v_rev_e - v_e) + (g_ie.T @ s_i) * (v_rev_i - v_e) + i_ext_e_local)
        ...
        v_i_inc = (0.1 * (-65 - v_i) + 9 * n_i ** 4 * (-90 - v_i) + 35 * m_i ** 3 * h_i * (55 - v_i)
                    + (g_ei.T @ s_e) * (v_rev_e - v_i) + (g_ii.T @ s_i) * (v_rev_i - v_i)
                    + i_ext_i + G_gap @ v_i - c_gap * v_i)
        ...
        v_i_inc = (0.1 * (-65 - v_i_tmp) + 9 * n_i_tmp ** 4 * (-90 - v_i_tmp)
                    ...
                    + (g_ei.T @ s_e_tmp) * (v_rev_e - v_i_tmp) + (g_ii.T @ s_i_tmp) * (v_rev_i - v_i_tmp)
                    + i_ext_i + G_gap @ v_i_tmp - c_gap * v_i_tmp)
...
t_e_spikes, i_e_spikes, t_i_spikes, i_i_spikes = simulate(i_ext_e)
