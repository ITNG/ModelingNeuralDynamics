...
if __name__ == "__main__":
    ...
    pl.plot(t, I_na, lw=2, c="r", label=r"$I_{na}$")
    pl.plot(t, I_k, lw=2, c="g", label=r"$I_{k}$")
    pl.plot(t, I_l, lw=2, c="b", label=r"$I_{l}$")
    pl.plot(t, I_tot, lw=2, c='k', label=r"$I_{tot}$")

    pl.legend()
    pl.xlim(min(t), max(t))
    pl.ylim(-20, 20)
    pl.xlabel("time [ms]", fontsize=15)
    pl.ylabel(r"$I [\mu A/cm^2]$", fontsize=15)
    pl.tight_layout()
    pl.tick_params(labelsize=15)
    pl.savefig("fig_7_3.png")
