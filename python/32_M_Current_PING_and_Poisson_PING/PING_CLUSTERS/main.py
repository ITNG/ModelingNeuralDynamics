...
sigma_i = 0.00
i_ext_i = 0.2 * np.ones(num_i) * (1 + sigma_i * rng.standard_normal(num_i))
g_hat_ee, g_hat_ei, g_hat_ie, g_hat_ii = 0., 5., 0.25, 0.75
p_ee, p_ei, p_ie, p_ii = 1., 1., 1., 1.

v_rev_e, v_rev_i = 0., -75.
tau_r_e, tau_peak_e, tau_d_e = 0.05, 0.05, 1.
tau_r_i, tau_peak_i, tau_d_i = 0.1, 0.1, 9.
t_final = 500.
dt = 0.01
dt05 = dt / 2
...
if __name__ == "__main__":
    ...
    if len(t_i_spikes) > 0:
        # matlab plots i-cells with 1-based indices (1..num_i); shift to match
        ax.plot(t_i_spikes, i_i_spikes + 1, '.b', markersize=6)
    if len(t_e_spikes) > 0:
        ax.plot(t_e_spikes, i_e_spikes + num_i, '.r', markersize=6)
    ax.plot([0, t_final], [num_i + 0.5, num_i + 0.5], '--k', linewidth=1)
    ax.set_yticks([50, 70])
    ax.axis([t_final - 200, t_final, 49.5, 70])
