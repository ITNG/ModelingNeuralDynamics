...
num_i = num_e // 4
sigma_e = 0.00
i_ext_e = 0.60 * np.ones(num_e) * (1 + sigma_e * rng.standard_normal(num_e))
sigma_i = 0.00
i_ext_i = 0.60 * np.ones(num_i) * (1 + sigma_i * rng.standard_normal(num_i))
g_hat_ee, g_hat_ei, g_hat_ie, g_hat_ii = 0., 1.25, 1.25, 0.4
p_ee, p_ei, p_ie, p_ii = 1.0, 1.0, 1.0, 1.0

v_rev_e, v_rev_i = 0., -75.
tau_r_e, tau_peak_e, tau_d_e = 0.3, 0.3, 3.
tau_r_i, tau_peak_i, tau_d_i = 0.3, 0.3, 9.
f_stoch, g_stoch = 40., 0.1  # parameters of the Poisson streams of
                               # excitatory synaptic inputs into the
                               # E-cells: mean frequency f_stoch, and
                               # maximum conductance g_stoch.
t_final = 500.
dt = 0.01
dt05 = dt / 2
...
for k in range(1, m_steps + 1):
    ...
    # Random Poisson input arrivals: u[i] < f_stoch/1000*dt with
    # probability f_stoch/1000*dt.
    u = rng.random(num_e)
    ind = u < f_stoch / 1000 * dt
    q_stoch[ind] = 1.

    spike_count[k - 1] = len(which_e)
    v_one[k] = v_e[2]
...
if __name__ == "__main__":
    f_hat_e = len(t_e_spikes) / num_e / t_final * 1000
    f_hat_i = len(t_i_spikes) / num_i / t_final * 1000
    print(f"f_hat_e = {f_hat_e}")
    print(f"f_hat_i = {f_hat_i}")

    # matlab's make_figure.m plots the rastergram+rate on figure(1) and
    # the voltage trace of a single E-cell on figure(2) (figure.pdf in
    # the matlab source only captured figure(2), the voltage trace).
    ...
    # smoothed E population firing rate, following matlab's rastergram.m:
    # a running sum of spike counts over a 10 ms sliding window.
    win = round(5 / dt)
    f_e = np.array([spike_count[k - win:k + win].sum() for k in range(win, m_steps - win)]) / 10
    f_e = f_e / num_e * 1000
    t_e = np.arange(win, m_steps - win) * dt - dt / 2
    axes[1].plot(t_e, f_e, '-k', linewidth=2)
    axes[1].set_xlabel('$t$ [ms]')
    axes[1].set_ylabel('$f_E$ [Hz]')

    # voltage trace of E-cell index 2 (matlab's v_e(3))
    axes[2].plot(np.arange(m_steps + 1) * dt, v_one, '-k', linewidth=2)
